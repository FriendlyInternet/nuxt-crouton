...
export const BRIEF_TYPES = [
  { emoji: '🔴', name: 'Issue', means: 'something is broken or wrong' },
  { emoji: '💡', name: 'Proposal', means: 'a suggestion you can take or drop' },
  { emoji: '✅', name: 'Done', means: 'landed and verified — no action needed' },
  { emoji: '🔀', name: 'Choice', means: 'a real fork, needs your pick' },
  { emoji: '🛠', name: 'Action', means: "you must do something the agent can't" },
  { emoji: '👀', name: 'Review', means: 'approve or reject a diff/preview' },
]
 ...
 */
export function checkOwnerBrief(body) {
...
export function explain(verdict) {
  const vocab = BRIEF_TYPES.map((t) => `  ${t.emoji}  ${t.name.padEnd(9)} ${t.means}`).join('\n')
  const why = {
