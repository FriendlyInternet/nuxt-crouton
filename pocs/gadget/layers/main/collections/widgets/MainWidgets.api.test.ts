...
vi.mock('./server/database/queries', () => ({
  getAllMainWidgets: vi.fn(),
  getMainWidgetsByIds: vi.fn(),
  createMainWidget: vi.fn(),
  updateMainWidget: vi.fn(),
  deleteMainWidget: vi.fn(),
}))
...
import getHandler from './server/api/teams/[id]/main-widgets/index.get.ts'
import postHandler from './server/api/teams/[id]/main-widgets/index.post.ts'
import patchHandler from './server/api/teams/[id]/main-widgets/[widgetId].patch.ts'
import deleteHandler from './server/api/teams/[id]/main-widgets/[widgetId].delete.ts'
...
describe('main/widgets API handlers (generated)', () => {

  describe('index.get', () => {
    it('rejects an unauthenticated request', async () => {
      unauth()
      await expect(getHandler({ __query: {} } as any)).rejects.toBeTruthy()
      expect(getAllMainWidgets).not.toHaveBeenCalled()
    })

    it('lists scoped to the resolved team', async () => {
      ;(getAllMainWidgets as any).mockResolvedValue([])
      await getHandler({ __query: {} } as any)
      expect(getAllMainWidgets).toHaveBeenCalled()
      // first positional arg is always the resolved team id (FK filters ride in opts)
      expect((getAllMainWidgets as any).mock.calls[0][0]).toBe(TEAM.id)
    })
  })
  ...
  describe('index.post', () => {
    it('rejects an unauthenticated request and never writes', async () => {
      ...
      expect(createMainWidget).not.toHaveBeenCalled()
    })

    it('creates scoped to the resolved team', async () => {
      ;(createMainWidget as any).mockResolvedValue({ id: 'rec_1' })
      const result = await postHandler({ __body: VALID_BODY } as any)
      expect(createMainWidget).toHaveBeenCalledWith(expect.objectContaining({ teamId: TEAM.id, owner: USER.id }))
  ...
  describe('[widgetId].patch', () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(patchHandler({ __params: { widgetId: 'rec_1' }, __body: {} } as any)).rejects.toBeTruthy()
      expect(updateMainWidget).not.toHaveBeenCalled()
    })

    it('updates scoped to the resolved team', async () => {
      ;(updateMainWidget as any).mockResolvedValue({ id: 'rec_1' })
      await patchHandler({ __params: { widgetId: 'rec_1' }, __body: {} } as any)
      expect(updateMainWidget).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything(), expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(updateMainWidget as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(patchHandler({ __params: { widgetId: 'missing' }, __body: {} } as any)).rejects.toMatchObject({ status: 404 })
    })
  })

  describe('[widgetId].delete', () => {
    it('400s when the id param is missing', async () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(deleteHandler({ __params: { widgetId: 'rec_1' } } as any)).rejects.toBeTruthy()
      expect(deleteMainWidget).not.toHaveBeenCalled()
    })

    it('deletes scoped to the resolved team', async () => {
      ;(deleteMainWidget as any).mockResolvedValue({ success: true })
      await deleteHandler({ __params: { widgetId: 'rec_1' } } as any)
      expect(deleteMainWidget).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(deleteMainWidget as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(deleteHandler({ __params: { widgetId: 'missing' } } as any)).rejects.toMatchObject({ status: 404 })
    })
  })
})
