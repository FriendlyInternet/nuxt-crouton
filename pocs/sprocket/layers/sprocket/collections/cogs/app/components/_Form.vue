  ...
  >
    <CroutonFormLayout>
      ...
      <div class="flex flex-col gap-4 p-1">
        <UFormField :label="t('sprocket.cogs.fields.name', 'Name')" name="name" class="not-last:pb-4">
          <UInput v-model="state.name" class="w-full" size="xl" />
        </UFormField>
        <UFormField :label="t('sprocket.cogs.fields.teeth', 'Teeth')" name="teeth" class="not-last:pb-4">
          <UInputNumber v-model="state.teeth" class="w-full" />
        </UFormField>
      </div>
      </template>
...
import type { SprocketCogFormProps, SprocketCogFormData } from '../../types'
import useSprocketCogs from '../composables/useSprocketCogs'

const props = defineProps<SprocketCogFormProps>()
const { defaultValue, schema, collection } = useSprocketCogs()
...
const state = ref<SprocketCogFormData & { id?: string | null }>(initialValues as SprocketCogFormData & { id?: string | null })
