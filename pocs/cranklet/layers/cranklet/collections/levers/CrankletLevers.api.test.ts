...
vi.mock('./server/database/queries', () => ({
  getAllCrankletLevers: vi.fn(),
  getCrankletLeversByIds: vi.fn(),
  createCrankletLever: vi.fn(),
  updateCrankletLever: vi.fn(),
  deleteCrankletLever: vi.fn(),
}))

import { resolveTeamAndCheckMembership } from '@fyit/crouton-auth/server/utils/team'
import {
  getAllCrankletLevers,
  createCrankletLever,
  updateCrankletLever,
  deleteCrankletLever,
} from './server/database/queries'
import getHandler from './server/api/teams/[id]/cranklet-levers/index.get.ts'
import postHandler from './server/api/teams/[id]/cranklet-levers/index.post.ts'
import patchHandler from './server/api/teams/[id]/cranklet-levers/[leverId].patch.ts'
import deleteHandler from './server/api/teams/[id]/cranklet-levers/[leverId].delete.ts'
...
describe('cranklet/levers API handlers (generated)', () => {

  describe('index.get', () => {
    it('rejects an unauthenticated request', async () => {
      unauth()
      await expect(getHandler({ __query: {} } as any)).rejects.toBeTruthy()
      expect(getAllCrankletLevers).not.toHaveBeenCalled()
    })

    it('lists scoped to the resolved team', async () => {
      ;(getAllCrankletLevers as any).mockResolvedValue([])
      await getHandler({ __query: {} } as any)
      expect(getAllCrankletLevers).toHaveBeenCalled()
      // first positional arg is always the resolved team id (FK filters ride in opts)
      expect((getAllCrankletLevers as any).mock.calls[0][0]).toBe(TEAM.id)
    })
  })
  ...
  describe('index.post', () => {
    it('rejects an unauthenticated request and never writes', async () => {
      ...
      expect(createCrankletLever).not.toHaveBeenCalled()
    })

    it('creates scoped to the resolved team', async () => {
      ;(createCrankletLever as any).mockResolvedValue({ id: 'rec_1' })
      const result = await postHandler({ __body: VALID_BODY } as any)
      expect(createCrankletLever).toHaveBeenCalledWith(expect.objectContaining({ teamId: TEAM.id, owner: USER.id }))
  ...
  describe('[leverId].patch', () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(patchHandler({ __params: { leverId: 'rec_1' }, __body: {} } as any)).rejects.toBeTruthy()
      expect(updateCrankletLever).not.toHaveBeenCalled()
    })

    it('updates scoped to the resolved team', async () => {
      ;(updateCrankletLever as any).mockResolvedValue({ id: 'rec_1' })
      await patchHandler({ __params: { leverId: 'rec_1' }, __body: {} } as any)
      expect(updateCrankletLever).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything(), expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(updateCrankletLever as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(patchHandler({ __params: { leverId: 'missing' }, __body: {} } as any)).rejects.toMatchObject({ status: 404 })
    })
  })

  describe('[leverId].delete', () => {
    it('400s when the id param is missing', async () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(deleteHandler({ __params: { leverId: 'rec_1' } } as any)).rejects.toBeTruthy()
      expect(deleteCrankletLever).not.toHaveBeenCalled()
    })

    it('deletes scoped to the resolved team', async () => {
      ;(deleteCrankletLever as any).mockResolvedValue({ success: true })
      await deleteHandler({ __params: { leverId: 'rec_1' } } as any)
      expect(deleteCrankletLever).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(deleteCrankletLever as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(deleteHandler({ __params: { leverId: 'missing' } } as any)).rejects.toMatchObject({ status: 404 })
    })
  })
})
