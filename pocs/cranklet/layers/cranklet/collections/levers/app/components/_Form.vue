  ...
  >
    <CroutonFormLayout>
      ...
      <div class="flex flex-col gap-4 p-1">
        <UFormField :label="t('cranklet.levers.fields.name', 'Name')" name="name" class="not-last:pb-4">
          <UInput v-model="state.name" class="w-full" size="xl" />
        </UFormField>
        <UFormField :label="t('cranklet.levers.fields.throw', 'Throw')" name="throw" class="not-last:pb-4">
          <UInputNumber v-model="state.throw" class="w-full" />
        </UFormField>
      </div>
      </template>
...
import type { CrankletLeverFormProps, CrankletLeverFormData } from '../../types'
import useCrankletLevers from '../composables/useCrankletLevers'

const props = defineProps<CrankletLeverFormProps>()
const { defaultValue, schema, collection } = useCrankletLevers()
...
const state = ref<CrankletLeverFormData & { id?: string | null }>(initialValues as CrankletLeverFormData & { id?: string | null })
