...
vi.mock('./server/database/queries', () => ({
  getAllMainPlates: vi.fn(),
  getMainPlatesByIds: vi.fn(),
  createMainPlate: vi.fn(),
  updateMainPlate: vi.fn(),
  deleteMainPlate: vi.fn(),
}))
...
import getHandler from './server/api/teams/[id]/main-plates/index.get.ts'
import postHandler from './server/api/teams/[id]/main-plates/index.post.ts'
import patchHandler from './server/api/teams/[id]/main-plates/[plateId].patch.ts'
import deleteHandler from './server/api/teams/[id]/main-plates/[plateId].delete.ts'
...
describe('main/plates API handlers (generated)', () => {

  describe('index.get', () => {
    it('rejects an unauthenticated request', async () => {
      unauth()
      await expect(getHandler({ __query: {} } as any)).rejects.toBeTruthy()
      expect(getAllMainPlates).not.toHaveBeenCalled()
    })

    it('lists scoped to the resolved team', async () => {
      ;(getAllMainPlates as any).mockResolvedValue([])
      await getHandler({ __query: {} } as any)
      expect(getAllMainPlates).toHaveBeenCalled()
      // first positional arg is always the resolved team id (FK filters ride in opts)
      expect((getAllMainPlates as any).mock.calls[0][0]).toBe(TEAM.id)
    })
  })
  ...
  describe('index.post', () => {
    it('rejects an unauthenticated request and never writes', async () => {
      ...
      expect(createMainPlate).not.toHaveBeenCalled()
    })

    it('creates scoped to the resolved team', async () => {
      ;(createMainPlate as any).mockResolvedValue({ id: 'rec_1' })
      const result = await postHandler({ __body: VALID_BODY } as any)
      expect(createMainPlate).toHaveBeenCalledWith(expect.objectContaining({ teamId: TEAM.id, owner: USER.id }))
  ...
  describe('[plateId].patch', () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(patchHandler({ __params: { plateId: 'rec_1' }, __body: {} } as any)).rejects.toBeTruthy()
      expect(updateMainPlate).not.toHaveBeenCalled()
    })

    it('updates scoped to the resolved team', async () => {
      ;(updateMainPlate as any).mockResolvedValue({ id: 'rec_1' })
      await patchHandler({ __params: { plateId: 'rec_1' }, __body: {} } as any)
      expect(updateMainPlate).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything(), expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(updateMainPlate as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(patchHandler({ __params: { plateId: 'missing' }, __body: {} } as any)).rejects.toMatchObject({ status: 404 })
    })
  })

  describe('[plateId].delete', () => {
    it('400s when the id param is missing', async () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(deleteHandler({ __params: { plateId: 'rec_1' } } as any)).rejects.toBeTruthy()
      expect(deleteMainPlate).not.toHaveBeenCalled()
    })

    it('deletes scoped to the resolved team', async () => {
      ;(deleteMainPlate as any).mockResolvedValue({ success: true })
      await deleteHandler({ __params: { plateId: 'rec_1' } } as any)
      expect(deleteMainPlate).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(deleteMainPlate as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(deleteHandler({ __params: { plateId: 'missing' } } as any)).rejects.toMatchObject({ status: 404 })
    })
  })
})
