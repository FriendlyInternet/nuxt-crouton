...
vi.mock('./server/database/queries', () => ({
  getAllMainBookmarks: vi.fn(),
  getMainBookmarksByIds: vi.fn(),
  createMainBookmark: vi.fn(),
  updateMainBookmark: vi.fn(),
  deleteMainBookmark: vi.fn(),
}))
...
import getHandler from './server/api/teams/[id]/main-bookmarks/index.get.ts'
import postHandler from './server/api/teams/[id]/main-bookmarks/index.post.ts'
import patchHandler from './server/api/teams/[id]/main-bookmarks/[bookmarkId].patch.ts'
import deleteHandler from './server/api/teams/[id]/main-bookmarks/[bookmarkId].delete.ts'
...
describe('main/bookmarks API handlers (generated)', () => {

  describe('index.get', () => {
    it('rejects an unauthenticated request', async () => {
      unauth()
      await expect(getHandler({ __query: {} } as any)).rejects.toBeTruthy()
      expect(getAllMainBookmarks).not.toHaveBeenCalled()
    })

    it('lists scoped to the resolved team', async () => {
      ;(getAllMainBookmarks as any).mockResolvedValue([])
      await getHandler({ __query: {} } as any)
      expect(getAllMainBookmarks).toHaveBeenCalled()
      // first positional arg is always the resolved team id (FK filters ride in opts)
      expect((getAllMainBookmarks as any).mock.calls[0][0]).toBe(TEAM.id)
    })
  })
  ...
  describe('index.post', () => {
    it('rejects an unauthenticated request and never writes', async () => {
      ...
      expect(createMainBookmark).not.toHaveBeenCalled()
    })

    it('creates scoped to the resolved team', async () => {
      ;(createMainBookmark as any).mockResolvedValue({ id: 'rec_1' })
      const result = await postHandler({ __body: VALID_BODY } as any)
      expect(createMainBookmark).toHaveBeenCalledWith(expect.objectContaining({ teamId: TEAM.id, owner: USER.id }))
  ...
  describe('[bookmarkId].patch', () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(patchHandler({ __params: { bookmarkId: 'rec_1' }, __body: {} } as any)).rejects.toBeTruthy()
      expect(updateMainBookmark).not.toHaveBeenCalled()
    })

    it('updates scoped to the resolved team', async () => {
      ;(updateMainBookmark as any).mockResolvedValue({ id: 'rec_1' })
      await patchHandler({ __params: { bookmarkId: 'rec_1' }, __body: {} } as any)
      expect(updateMainBookmark).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything(), expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(updateMainBookmark as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(patchHandler({ __params: { bookmarkId: 'missing' }, __body: {} } as any)).rejects.toMatchObject({ status: 404 })
    })
  })

  describe('[bookmarkId].delete', () => {
    it('400s when the id param is missing', async () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(deleteHandler({ __params: { bookmarkId: 'rec_1' } } as any)).rejects.toBeTruthy()
      expect(deleteMainBookmark).not.toHaveBeenCalled()
    })

    it('deletes scoped to the resolved team', async () => {
      ;(deleteMainBookmark as any).mockResolvedValue({ success: true })
      await deleteHandler({ __params: { bookmarkId: 'rec_1' } } as any)
      expect(deleteMainBookmark).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(deleteMainBookmark as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(deleteHandler({ __params: { bookmarkId: 'missing' } } as any)).rejects.toMatchObject({ status: 404 })
    })
  })
})
