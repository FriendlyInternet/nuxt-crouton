...
vi.mock('./server/database/queries', () => ({
  getAllMainAwards: vi.fn(),
  getMainAwardsByIds: vi.fn(),
  createMainAward: vi.fn(),
  updateMainAward: vi.fn(),
  deleteMainAward: vi.fn(),
}))
...
import getHandler from './server/api/teams/[id]/main-awards/index.get.ts'
import postHandler from './server/api/teams/[id]/main-awards/index.post.ts'
import patchHandler from './server/api/teams/[id]/main-awards/[awardId].patch.ts'
import deleteHandler from './server/api/teams/[id]/main-awards/[awardId].delete.ts'
...
describe('main/awards API handlers (generated)', () => {

  describe('index.get', () => {
    it('rejects an unauthenticated request', async () => {
      unauth()
      await expect(getHandler({ __query: {} } as any)).rejects.toBeTruthy()
      expect(getAllMainAwards).not.toHaveBeenCalled()
    })

    it('lists scoped to the resolved team', async () => {
      ;(getAllMainAwards as any).mockResolvedValue([])
      await getHandler({ __query: {} } as any)
      expect(getAllMainAwards).toHaveBeenCalled()
      // first positional arg is always the resolved team id (FK filters ride in opts)
      expect((getAllMainAwards as any).mock.calls[0][0]).toBe(TEAM.id)
    })
  })
  ...
  describe('index.post', () => {
    it('rejects an unauthenticated request and never writes', async () => {
      ...
      expect(createMainAward).not.toHaveBeenCalled()
    })

    it('creates scoped to the resolved team', async () => {
      ;(createMainAward as any).mockResolvedValue({ id: 'rec_1' })
      const result = await postHandler({ __body: VALID_BODY } as any)
      expect(createMainAward).toHaveBeenCalledWith(expect.objectContaining({ teamId: TEAM.id, owner: USER.id }))
  ...
  describe('[awardId].patch', () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(patchHandler({ __params: { awardId: 'rec_1' }, __body: {} } as any)).rejects.toBeTruthy()
      expect(updateMainAward).not.toHaveBeenCalled()
    })

    it('updates scoped to the resolved team', async () => {
      ;(updateMainAward as any).mockResolvedValue({ id: 'rec_1' })
      await patchHandler({ __params: { awardId: 'rec_1' }, __body: {} } as any)
      expect(updateMainAward).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything(), expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(updateMainAward as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(patchHandler({ __params: { awardId: 'missing' }, __body: {} } as any)).rejects.toMatchObject({ status: 404 })
    })
  })

  describe('[awardId].delete', () => {
    it('400s when the id param is missing', async () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(deleteHandler({ __params: { awardId: 'rec_1' } } as any)).rejects.toBeTruthy()
      expect(deleteMainAward).not.toHaveBeenCalled()
    })

    it('deletes scoped to the resolved team', async () => {
      ;(deleteMainAward as any).mockResolvedValue({ success: true })
      await deleteHandler({ __params: { awardId: 'rec_1' } } as any)
      expect(deleteMainAward).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(deleteMainAward as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(deleteHandler({ __params: { awardId: 'missing' } } as any)).rejects.toMatchObject({ status: 404 })
    })
  })
})
