...
vi.mock('./server/database/queries', () => ({
  getAllChoresChores: vi.fn(),
  getChoresChoresByIds: vi.fn(),
  createChoresChore: vi.fn(),
  updateChoresChore: vi.fn(),
  deleteChoresChore: vi.fn(),
}))
...
import getHandler from './server/api/teams/[id]/chores-chores/index.get.ts'
import postHandler from './server/api/teams/[id]/chores-chores/index.post.ts'
import patchHandler from './server/api/teams/[id]/chores-chores/[choreId].patch.ts'
import deleteHandler from './server/api/teams/[id]/chores-chores/[choreId].delete.ts'
...
describe('chores/chores API handlers (generated)', () => {

  describe('index.get', () => {
    it('rejects an unauthenticated request', async () => {
      unauth()
      await expect(getHandler({ __query: {} } as any)).rejects.toBeTruthy()
      expect(getAllChoresChores).not.toHaveBeenCalled()
    })

    it('lists scoped to the resolved team', async () => {
      ;(getAllChoresChores as any).mockResolvedValue([])
      await getHandler({ __query: {} } as any)
      expect(getAllChoresChores).toHaveBeenCalled()
      // first positional arg is always the resolved team id (FK filters ride in opts)
      expect((getAllChoresChores as any).mock.calls[0][0]).toBe(TEAM.id)
    })
  })
  ...
  describe('index.post', () => {
    it('rejects an unauthenticated request and never writes', async () => {
      ...
      expect(createChoresChore).not.toHaveBeenCalled()
    })

    it('creates scoped to the resolved team', async () => {
      ;(createChoresChore as any).mockResolvedValue({ id: 'rec_1' })
      const result = await postHandler({ __body: VALID_BODY } as any)
      expect(createChoresChore).toHaveBeenCalledWith(expect.objectContaining({ teamId: TEAM.id, owner: USER.id }))
  ...
  describe('[choreId].patch', () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(patchHandler({ __params: { choreId: 'rec_1' }, __body: {} } as any)).rejects.toBeTruthy()
      expect(updateChoresChore).not.toHaveBeenCalled()
    })

    it('updates scoped to the resolved team', async () => {
      ;(updateChoresChore as any).mockResolvedValue({ id: 'rec_1' })
      await patchHandler({ __params: { choreId: 'rec_1' }, __body: {} } as any)
      expect(updateChoresChore).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything(), expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(updateChoresChore as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(patchHandler({ __params: { choreId: 'missing' }, __body: {} } as any)).rejects.toMatchObject({ status: 404 })
    })
  })

  describe('[choreId].delete', () => {
    it('400s when the id param is missing', async () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(deleteHandler({ __params: { choreId: 'rec_1' } } as any)).rejects.toBeTruthy()
      expect(deleteChoresChore).not.toHaveBeenCalled()
    })

    it('deletes scoped to the resolved team', async () => {
      ;(deleteChoresChore as any).mockResolvedValue({ success: true })
      await deleteHandler({ __params: { choreId: 'rec_1' } } as any)
      expect(deleteChoresChore).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(deleteChoresChore as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(deleteHandler({ __params: { choreId: 'missing' } } as any)).rejects.toMatchObject({ status: 404 })
    })
  })
})
