...
| `src/runtime/components/SpecWalkOverlay.vue` | The non-blocking bottom-sheet walk panel — steps, `data-handoff` outline tracker (rAF), verdicts, `lgtm <id>` Export. Standard-utility styling (no arbitrary Tailwind — see the gotcha above). **Minimizable (#1180 follow-up):** a chevron tucks the sheet to a compact pill (`data-handoff="spec-walk-min"`) so you can test the app, then tap to restore — state is module-singleton + localStorage, so nothing is lost. Readable walk text (base-size behaviour, un-muted steps). |
