...
| `app/utils/voice-order.ts` | Talk-to-order parser (#1429) — pure logic mapping a spoken nl utterance ("twee pils en een koffie") onto the event's products as `{ lines, unmatched }` draft cart lines. Dutch number words 1–20 + tens, plural/diminutive stemming, 1-edit STT fuzz; below-confidence segments go to `unmatched`, never guessed. Behaviour contract: `test/voice-order.test.ts` |
| `app/composables/useVoiceOrder.ts` | Talk-to-order speech capture (#1429) — wraps VueUse `useSpeechRecognition` (on-device Web Speech API, `nl-BE`, one utterance per press) and feeds each final utterance through `parseVoiceOrder`. The ONLY module touching the STT engine — swap point for a server transcriber later |
...
- **"Data"** (admin-only: `useAuth().loggedIn` — PIN helpers hold a scoped token, not a session,
  ...
  The chart/matrix endpoints are team-members-only, matching the gate.
