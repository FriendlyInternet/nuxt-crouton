...
preview URL on a draft PR, and hold on `status:needs-input`. The reviewer opens the running page,
...
on `status:needs-input`; approval is a **comment** containing `lgtm`/`approve` (not a reaction or label,
