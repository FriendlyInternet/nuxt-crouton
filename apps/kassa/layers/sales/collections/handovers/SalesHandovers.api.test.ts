...
vi.mock('./server/database/queries', () => ({
  getAllSalesHandovers: vi.fn(),
  getSalesHandoversByIds: vi.fn(),
  createSalesHandover: vi.fn(),
  updateSalesHandover: vi.fn(),
  deleteSalesHandover: vi.fn(),
}))

import { resolveTeamAndCheckMembership } from '@fyit/crouton-auth/server/utils/team'
import {
  getAllSalesHandovers,
  createSalesHandover,
  updateSalesHandover,
  deleteSalesHandover,
} from './server/database/queries'
import getHandler from './server/api/teams/[id]/sales-handovers/index.get.ts'
import postHandler from './server/api/teams/[id]/sales-handovers/index.post.ts'
import patchHandler from './server/api/teams/[id]/sales-handovers/[handoverId].patch.ts'
import deleteHandler from './server/api/teams/[id]/sales-handovers/[handoverId].delete.ts'
...
describe('sales/handovers API handlers (generated)', () => {

  describe('index.get', () => {
    it('rejects an unauthenticated request', async () => {
      unauth()
      await expect(getHandler({ __query: {} } as any)).rejects.toBeTruthy()
      expect(getAllSalesHandovers).not.toHaveBeenCalled()
    })

    it('lists scoped to the resolved team', async () => {
      ;(getAllSalesHandovers as any).mockResolvedValue([])
      await getHandler({ __query: {} } as any)
      expect(getAllSalesHandovers).toHaveBeenCalled()
      // first positional arg is always the resolved team id (FK filters ride in opts)
      expect((getAllSalesHandovers as any).mock.calls[0][0]).toBe(TEAM.id)
    })
  })
  ...
  describe('index.post', () => {
    it('rejects an unauthenticated request and never writes', async () => {
      ...
      expect(createSalesHandover).not.toHaveBeenCalled()
    })

    it('creates scoped to the resolved team', async () => {
      ;(createSalesHandover as any).mockResolvedValue({ id: 'rec_1' })
      const result = await postHandler({ __body: VALID_BODY } as any)
      expect(createSalesHandover).toHaveBeenCalledWith(expect.objectContaining({ teamId: TEAM.id, owner: USER.id }))
  ...
  describe('[handoverId].patch', () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(patchHandler({ __params: { handoverId: 'rec_1' }, __body: {} } as any)).rejects.toBeTruthy()
      expect(updateSalesHandover).not.toHaveBeenCalled()
    })

    it('updates scoped to the resolved team', async () => {
      ;(updateSalesHandover as any).mockResolvedValue({ id: 'rec_1' })
      await patchHandler({ __params: { handoverId: 'rec_1' }, __body: {} } as any)
      expect(updateSalesHandover).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything(), expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(updateSalesHandover as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(patchHandler({ __params: { handoverId: 'missing' }, __body: {} } as any)).rejects.toMatchObject({ status: 404 })
    })
  })

  describe('[handoverId].delete', () => {
    it('400s when the id param is missing', async () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(deleteHandler({ __params: { handoverId: 'rec_1' } } as any)).rejects.toBeTruthy()
      expect(deleteSalesHandover).not.toHaveBeenCalled()
    })

    it('deletes scoped to the resolved team', async () => {
      ;(deleteSalesHandover as any).mockResolvedValue({ success: true })
      await deleteHandler({ __params: { handoverId: 'rec_1' } } as any)
      expect(deleteSalesHandover).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(deleteSalesHandover as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(deleteHandler({ __params: { handoverId: 'missing' } } as any)).rejects.toMatchObject({ status: 404 })
    })
  })
})
