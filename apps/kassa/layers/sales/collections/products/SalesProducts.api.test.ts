...
vi.mock('./server/database/queries', () => ({
  getAllSalesProducts: vi.fn(),
  getSalesProductsByIds: vi.fn(),
  createSalesProduct: vi.fn(),
  updateSalesProduct: vi.fn(),
  deleteSalesProduct: vi.fn(),
  reorderSiblingsSalesProducts: vi.fn(),
}))
...
import {
  ...
  updateSalesProduct,
  deleteSalesProduct,
  reorderSiblingsSalesProducts,
} from './server/database/queries'
import getHandler from './server/api/teams/[id]/sales-products/index.get.ts'
import postHandler from './server/api/teams/[id]/sales-products/index.post.ts'
import patchHandler from './server/api/teams/[id]/sales-products/[productId].patch.ts'
import deleteHandler from './server/api/teams/[id]/sales-products/[productId].delete.ts'
import reorderHandler from './server/api/teams/[id]/sales-products/reorder.patch.ts'
...
describe('sales/products API handlers (generated)', () => {

  describe('index.get', () => {
    it('rejects an unauthenticated request', async () => {
      unauth()
      await expect(getHandler({ __query: {} } as any)).rejects.toBeTruthy()
      expect(getAllSalesProducts).not.toHaveBeenCalled()
    })

    it('lists scoped to the resolved team', async () => {
      ;(getAllSalesProducts as any).mockResolvedValue([])
      await getHandler({ __query: {} } as any)
      expect(getAllSalesProducts).toHaveBeenCalled()
      // first positional arg is always the resolved team id (FK filters ride in opts)
      expect((getAllSalesProducts as any).mock.calls[0][0]).toBe(TEAM.id)
    })
  })
  ...
  describe('index.post', () => {
    it('rejects an unauthenticated request and never writes', async () => {
      ...
      expect(createSalesProduct).not.toHaveBeenCalled()
    })

    it('creates scoped to the resolved team', async () => {
      ;(createSalesProduct as any).mockResolvedValue({ id: 'rec_1' })
      const result = await postHandler({ __body: VALID_BODY } as any)
      expect(createSalesProduct).toHaveBeenCalledWith(expect.objectContaining({ teamId: TEAM.id, owner: USER.id }))
      expect(result).toMatchObject({ id: 'rec_1' })
    })

    it('rejects an invalid body before writing', async () => {
      // required `eventId` omitted → readValidatedBody throws → no write happens
      await expect(postHandler({ __body: INVALID_BODY } as any)).rejects.toBeTruthy()
      expect(createSalesProduct).not.toHaveBeenCalled()
    })
  })

  describe('[productId].patch', () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(patchHandler({ __params: { productId: 'rec_1' }, __body: {} } as any)).rejects.toBeTruthy()
      expect(updateSalesProduct).not.toHaveBeenCalled()
    })

    it('updates scoped to the resolved team', async () => {
      ;(updateSalesProduct as any).mockResolvedValue({ id: 'rec_1' })
      await patchHandler({ __params: { productId: 'rec_1' }, __body: {} } as any)
      expect(updateSalesProduct).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything(), expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(updateSalesProduct as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(patchHandler({ __params: { productId: 'missing' }, __body: {} } as any)).rejects.toMatchObject({ status: 404 })
    })
  })
  ...
  describe('[productId].delete', () => {
    ...
    it('rejects an unauthenticated request and never writes', async () => {
      unauth()
      await expect(deleteHandler({ __params: { productId: 'rec_1' } } as any)).rejects.toBeTruthy()
      expect(deleteSalesProduct).not.toHaveBeenCalled()
    })

    it('deletes scoped to the resolved team', async () => {
      ;(deleteSalesProduct as any).mockResolvedValue({ success: true })
      await deleteHandler({ __params: { productId: 'rec_1' } } as any)
      expect(deleteSalesProduct).toHaveBeenCalledWith('rec_1', TEAM.id, USER.id, expect.anything())
    })

    it('propagates a not-found from the query as a 404', async () => {
      ;(deleteSalesProduct as any).mockRejectedValue(Object.assign(new Error('not found'), { status: 404 }))
      await expect(deleteHandler({ __params: { productId: 'missing' } } as any)).rejects.toMatchObject({ status: 404 })
    })
  })
  ...
  describe('reorder.patch', () => {
    it('rejects an unauthenticated request', async () => {
      ...
      expect(reorderSiblingsSalesProducts).not.toHaveBeenCalled()
    })
    ...
    it('reorders scoped to the resolved team', async () => {
      ;(reorderSiblingsSalesProducts as any).mockResolvedValue([])
      await reorderHandler({ __body: { updates: [{ id: 'a', order: 0 }] } } as any)
      expect((reorderSiblingsSalesProducts as any).mock.calls[0][0]).toBe(TEAM.id)
    })
  })
})
