...
|---|---|---|---|
| `No schema files found for path config ['.nuxt/hub/db/schema.mjs']` from `db:generate` | the bundled schema `.mjs` exists only after `nuxt build` | **RETIRED BY DESIGN on resolver apps** (#1445 WS2): a current `drizzle.config.ts` calls `resolveSchemaSourcesSync` and never reads `.nuxt`, so this string cannot occur. If you see it, the app is still on the legacy bundle (`apps/triage` until #1456) → build-first recipe, canonical: `db-migrations` skill | #523/#1445; `packages/crouton-cli/lib/utils/schema-sources.ts` |
