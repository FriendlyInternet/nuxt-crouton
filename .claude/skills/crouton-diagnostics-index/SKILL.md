...
|---|---|---|---|
| `No schema files found for path config ['.nuxt/hub/db/schema.mjs']` from `db:generate` | the bundled schema `.mjs` exists only after `nuxt build` | **RETIRED BY DESIGN on resolver apps** (#1445 WS2): a current `drizzle.config.ts` calls `resolveSchemaSourcesSync` and never reads `.nuxt`, so this string cannot occur. All three apps here are on the resolver since #1456, so seeing this means an external/older scaffold → build-first recipe, canonical: `db-migrations` skill | #523/#1445; `packages/crouton-cli/lib/utils/schema-sources.ts` |
...
| `No migrations present` on a FRESH scaffold's first deploy | old CLI ran `db:generate` with no build → silently zero migrations | **Cause changed (#1445 WS2).** The CLI no longer builds at all — it resolves the graph directly. On a cold scaffold `crouton config` **soft-defers** the migration (`↻ Migration deferred … re-run: pnpm db:generate`, exit 0, #1457); if that line was missed, no migration exists to deploy. Fix: run `pnpm db:generate` and **commit** the result. Legacy `.nuxt`-bundle apps still need the build-first recipe — canonical: `db-migrations` skill | #523/#457/#1445/#1457; why: `crouton-failure-archaeology` §B |
...
grep -n "drizzle-orm" apps/kassa/nuxt.config.ts                      # #798 inline still in place? (was :93-101)
