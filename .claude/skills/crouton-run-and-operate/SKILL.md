...
`crouton-seed` (bin: `packages/crouton-cli/bin/crouton-seed.mjs`) discovers `SeedProvider`s from every extended `@fyit/crouton-*` package, emits idempotent upsert SQL, and executes it — **local → straight into `<app>/.data/db/sqlite.db` via libsql** (the file `nuxt dev` reads; #1612), **remote → `npx wrangler d1 execute <db> --remote`**. Flags (verified): `--db` (required) `--remote` `--dir` `--team` (default `test1`) `--locale` (default `nl`) `--with-staff` `--dry-run`. It also seeds the app's default layout: `crouton.layout.json` → a `layout_configs` row with id `default` (#709; `lib/seed-app.ts`).

**✅ THE seed-visibility trap is FIXED (#1612).** Historically a local `crouton-seed` shelled to `wrangler d1 execute --local`, which writes the **miniflare** DB (`<app>/.wrangler/state/v3/d1/miniflare-D1DatabaseObject/*.sqlite`), while `nuxt dev` with `hub: { db: 'sqlite' }` reads a **different** file (`<app>/.data/db/sqlite.db`) — so locally-seeded data never appeared in the dev app, with no copy step to bridge them. As of #1612 the `--local` path executes the seed SQL **directly against `<app>/.data/db/sqlite.db`** via `@libsql/client` (the same driver dev uses) — no split, no manual copy. **Precondition:** that DB must exist with migrations applied (`pnpm dev` once, then `pnpm db:migrate`); otherwise the seed errors with an actionable message. (`crouton db-pull` still bridges its *own* miniflare→`.data` import via `lib/db-pull.ts` + `_hub_migrations` — a separate flow.) If a *local* seed still "doesn't show up", check that `.data/db/sqlite.db` exists and migrations ran, not the old split.
