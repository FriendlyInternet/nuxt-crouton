...
| `.claude/settings.json` | Wires the hooks (PreToolUse `Edit\|Write` → `gate-package-edits.sh` + `gate-spec-signoff.mjs`; PreToolUse `mcp__github__add_issue_comment` → `require-comment-provenance.mjs`; SessionStart → `session-start.sh`) + the `mcpServers` entries | The Claude Code harness itself | Known stale `mcpServers.crouton` path → silent no-op, see #1098 |
| `.claude/hooks/pre-commit-sync-reminder` | Optional **git** pre-commit reminder (manual install; not wired by settings.json) | git, only if a human ran `cp` / `core.hooksPath` | Greps long-renamed package dirs — silent no-op, see #1098 |
...
| `nuxt.config.ts` `routeRules` | ISR/SWR/prerender caching per route | Nitro | ⚠️ **Do NOT add wildcard `/api/teams/*/...` rules** — breaks ALL generated `/api/teams/:id/*` routes via a radix3 conflict (live warning comment in `apps/velo/nuxt.config.ts`); root CLAUDE.md's ISR example recommends exactly the broken pattern. Full story: sibling `crouton-diagnostics-index` |
...
| `scripts/harness-layers.mjs` | `layer:` frontmatter of every skill/agent → inventory (`--json`); legacy no-frontmatter skills via its hardcoded `OVERRIDES` | **NONE — despite its own header claiming CI enforcement** (see #1098) |
