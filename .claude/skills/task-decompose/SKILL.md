...
4. **Block, don't improvise (#352, #1229).** A worker that finds a prerequisite missing (a
   ...
   exact error + what you tried + what you'd need, `@mention @pmcp`, set `status:needs-input`, stop —
...
- Post a plain top-level issue comment (never a PR *review* body) naming the pending patch and
  @mentioning `NOTIFY_HANDLE`. Add `status:needs-input` only if the omitted workflow change is
...
- **Automatic — resume after a blocker:** when an agent @mentions you and sets
  `status:needs-input`, just **reply on the issue** — `.github/workflows/resume-on-comment.yml`
...
- **Canonical workstream order for a crouton app — use the CLI, NEVER hand-mirror.** A crouton app build decomposes into this fixed shape. Do not reinvent it, and do **not** tell a worker to "mirror `pocs/blog`" or hand-copy another app — that is how versions drift:
  1. **Design the data model + generation plan, then SIGN OFF.** Draft both `schemas/*.json` (the fields) **and** `crouton.config.js` (collections, target `layer`, `features`, `locales`, `dialect`). Run the **`schema-review`** skill and get human sign-off on **both** before any code is generated — the config decides the forms / list views / API surface, so it is reviewed *alongside* the schema, not after. **Post the review, the `@mention`, and `status:needs-input` on this workstream's OWN issue (or its draft PR) — NEVER on the epic.** This *is* the schema/config sign-off issue, so the field tables, the config, the ping, and the human `approve` reply all live in one place; the epic only carries the final verify rollup. (If a step would touch `packages/`, the HARD GATE applies — but a normal POC build never does.)
...
- **Real blocker / decision needed** → `add_issue_comment` on the issue with a tight
  question + options, **@mention `NOTIFY_HANDLE`**, apply the `status:needs-input` label,
...
review/analysis can live in its own artifact; the `@mention` + `status:needs-input` ask stands alone
on top.
