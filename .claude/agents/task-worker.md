...
## `.github/workflows/` boundary — declare-loudly (#2108/#2109, supersedes #1076)

The Harness App token was granted the `workflows` scope (#2108/#2109) — the old embed-patch
convention (#1076, when the token couldn't write `.github/workflows/**` at all) is retired.
You **may now commit workflow-file changes directly**, in the same PR as everything else.

When your leaf's work requires a workflow-file change (a new trigger, a path filter, a new job):

- **Commit it normally**, alongside your other changes — no more omitting workflow-touching
  lines from the diff.
- **Declare it loudly.** Either add the `workflows:approved` label to the PR, or open the PR
  body with a `Workflow-change: <why>` line. This is the same declare-don't-prohibit stance as
  the `packages/` approval gate (`packages-guard.yml`) — the point is visibility, not permission.
- **A workflow-touching PR is NEVER auto-merged.** The automerge workflow excludes any PR whose
  diff touches `.github/workflows/**` — it still requires a human to merge it. Declare-loudly,
  not self-merge.
- **Fallback:** if for some reason you cannot write the workflow file directly, the old
  embed-a-patch-in-the-PR-body approach (a `## Workflow patch (human applies)` heading, a
  `git apply`-able fenced diff, `add_issue_comment` + @mention the notify handle, `status:blocked`
  only if load-bearing) remains a legitimate fallback deliverable.
