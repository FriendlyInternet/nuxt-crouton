...
You can add metadata properties like `required` for validation, `label` for human-readable form labels, `maxLength` for string length limits, `component` to specify a custom input component, `readOnly` to display reference fields as non-editable cards (useful for audit fields), `area` to logically group fields for form layout, `group` to create tabbed groups when multiple main groups exist, `nullable` (legacy no-op — every non-required field already validates `.nullish()`), and `translatable` to mark fields for i18n support.
