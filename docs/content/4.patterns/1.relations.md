...
- **Type-to-create (opt-in)** - With the `inline-create` prop, typing a name that
  doesn't match any item shows an "Add "\<name\>"" row; selecting it (or pressing
  Enter) creates the item inline with the typed label (plus `createInitialData`)
  and selects it — no slideover form. Only enable it for collections where a bare
  label is a valid create payload.
